...
public class TokenAuthentificationService {
	 ...
	 */
	private static final Logger LOG = LoggerFactory.getLogger(TokenAuthentificationService.class);
	/**
	 * Requete http.
	 */
	@Context
	...
	@POST
	@Path("login")
	public LoginResponse creerToken(@Context HttpServletRequest req, User login) throws ServletException {
		...
		try {
			req.login(login.getName(), login.getPassword());
		...
		LoginResponse loginResponse = new LoginResponse(Jwts.builder().setSubject(login.getName())
			.claim("roles", userDb.get(login.getName())).setIssuedAt(new Date()).signWith(SignatureAlgorithm.HS256, "secretkey")
			.compact());
	...
	@GET
	@Path("token")
	public TokenValidity validerToken(@HeaderParam("Authorization") String authString) throws ServletException {
