package com.rmc.authentification.utils;
...
public class AuthentificationFiltre implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response,
		...
		String authHeader = httpRequest.getHeader(AuthentificationUtils.AUTH_HEADER_KEY);
		...
		if (StringUtils.isBlank(authHeader) || authHeader.split(" ").length != 2) {
			...
			try {
				claimSet = (JWTClaimsSet) AuthentificationUtils.decodeToken(authHeader);
